...
public class LoginDAO {
    ...
    public ArrayList<Login> verificaUsuarioseSenha(){
        ...
        try ( PreparedStatement queryPesquisaCliente = this.conn
            ...
            login.setUsuarioLogin(resultSet.getString("usuario"));
            login.setSenhaLogin(resultSet.getString("senha"));
            login.setAtivoLogin(resultSet.getBoolean("ativo"));
        ...
        try ( PreparedStatement queryAlteraLogin = this.conn
            ...
            queryAlteraLogin.setString(1, usuarioAlterado);
            queryAlteraLogin.setString(2, senha);
    ...
    public void desativarLogin(String usuario){
        
        try ( PreparedStatement queryexcluirLogin = this.conn
                .prepareStatement("UPDATE login SET ativo = 0 "
                        + "WHERE usuario = ?")){
    ...
    public int pegaIdLogin(String usuario){
        
        ResultSet resultSet;
        Integer id = null;
        
        try ( PreparedStatement queryPegaIdLogin = this.conn
                .prepareStatement("SELECT id FROM login "
                        + "WHERE usuario = ?")){
          
           queryPegaIdLogin.setString(1, usuario);
           resultSet = queryPegaIdLogin.executeQuery();
    ...
    public Login pesquisaLoginsAtivo(String usuario){
        
        ResultSet resultSet;
        Login login = new Login();
        
        try ( PreparedStatement querypesquisaLoginsAtivo = this.conn
                .prepareStatement("SELECT ativo FROM login "
                        + "WHERE usuario = ?")){
            
            querypesquisaLoginsAtivo.setString(1, usuario);
            
            resultSet = querypesquisaLoginsAtivo.executeQuery();
               
            login.setAtivoLogin(resultSet.getBoolean("ativo"));
            
            querypesquisaLoginsAtivo.close();
            resultSet.close();
    ...
    public void ativarLogin(String usuario){
        
        try ( PreparedStatement queryexcluirLogin = this.conn
                .prepareStatement("UPDATE login SET ativo = 1 "
                        + "WHERE usuario = ?")){
